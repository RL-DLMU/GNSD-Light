...
parser.add_argument('--ngpu', type=str, default="0,1", help='gpu to be used')  # choose gpu card
...
parser.add_argument('-a', '--agent', type=str, default="GNSD-Light", help="agent type of agents in RL environment")
    ...
    def config_registry(self):
        ...
        if self.config['model'].get('graphic', True):
            ...
            if self.config['command']['world'] in ['cityflow', 'sumo']:
                if self.config['command']['world'] == 'sumo':
